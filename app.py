    ...
    if request.method == 'POST':
        ...
        if file and pin:
            ...
            token = save_file_info(
                session['username'], fname, path, pin,
                int(request.form.get('download_limit', 5)),
                'auto_delete' in request.form
            )
...
def generate_qr(token):
    ...
    # Для админа — разрешаем любой файл
    ...
    try:
        ...
        return send_file(buffered, mimetype='image/png', as_attachment=False)

    except Exception as e:
        # Выводим ошибку в браузер для отладки
